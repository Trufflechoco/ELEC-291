...
void main() {
	...
	if (P1_1 == 1) { //P1.1 is connected to AT89 Microcontroller (Reference Waveform)

		sleep((int)Period1/4 * 1000);
		PeakVoltage1 = currentVoltage;
	...
	if (P1_2 == 1) { //P1.2 is connected to AT89 Microcontroller (Reference Waveform)

		sleep((int)Period2 * 1000);
		PeakVoltage2 = currentVoltage;


	}
	//Calculate time difference between two waves
	timeDifference = math.abs(Period2 / 4.0 - Period1 / 4.0);

	//Convert peak voltage to Vrms and display it
	RMSVoltage1 = PeakVoltage1 * 0.7071;
	printf("%lf\n", RMSVoltage1);
	RMSVoltage2 = PeakVoltage2 * 0.7071;
	printf("%lf\n", RMSVoltage2);

	//Calculate Phase Shift and display it
	PhaseShift1 = (360.0 * timeDifference / Period1);
	printf("%lf\n", PhaseShift1);
	PhaseShift1 = (360.0 * timeDifference / Period2);
	printf("%lf\n", PhaseShift2);

	//Convert to Frequency and Display
...
{
	AUXR = 0B_0001_0001; // 1152 bytes of internal XDATA, P4.4 is a general purpose
	I / O
		P0M0 = 0x00; P0M1 = 0x00;
	P1M0 = 0x00; P1M1 = 0x00;
	P2M0 = 0x00; P2M1 = 0x00;
...
unsigned int volatile GetADC(unsigned char channel)
{
	...
	spid = SPIWrite((channel * 0x10) | 0x80); //Send single/diff* bit, D2, D1, and D0 
	bits.
		adc = ((spid & 0x03) * 0x100);// spid has the two most significant bits of the 
	result.
